...
void setup() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESP32Client", mqttUser, mqttPassword )) {
      ...
      display.clearDisplay();
      print_line("MQTT Broker Connected!",0,20,2);
      Serial.println("connected");
 
    } else {
 
      display.clearDisplay();
      print_line("MQTT Broker Failed!",0,20,2);
...
void print_time_now(void){
  ...
  display.clearDisplay();
  //print_line(String(days), 0, 0, 1);
  //print_line(" - ", 20, 0, 2);
 // print_line(String (hours), 30, 20, 1);
  //print_line(":", 50, 20, 1);
  //print_line(String(minutes),60, 20, 1);
  //print_line(" :", 70, 20, 1);
  //print_line(String(seconds),90, 20, 1);
...
void check_temp(){
  ...
  client.loop();
  sprintf(buf, " Sanjula_%d_24C", i);
  sprintf(buf2," %d",i);
  print_line(buf2,0,25,4);
